...
function extractPptxText(pptxPath) {
  // Prefer python-pptx text extraction (fast, deterministic). Fall back to
  // markitdown if python-pptx is unavailable.
  const script = `
import sys
try:
  ...
  for (let i = 1; i <= SLIDE_COUNT; i++) {
    ...
    try {
      ...
      if (typeof mod.createSlide !== 'function') {
        console.error(`[compile] slide-${num}.js has no createSlide(pres, theme) export`);
        continue;
      }
