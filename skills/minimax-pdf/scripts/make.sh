...
#   fix                            Print dependency install guidance
...
cmd_check() {
  ...
  if python3 -c "import reportlab" 2>/dev/null; then
    ...
    yellow "  ⚠ reportlab not installed  (run: make.sh fix for guidance)"
    ok=false
  fi
  ...
  if python3 -c "import pypdf" 2>/dev/null; then
    ...
    yellow "  ⚠ pypdf not installed  (run: make.sh fix for guidance)"
    ok=false
  fi
  ...
  if node -e "require('playwright')" 2>/dev/null; then
    ...
    yellow "  ⚠ playwright not found  (run: make.sh fix for guidance)"
    ok=false
  fi
  ...
  if python3 -c "import matplotlib" 2>/dev/null; then
    ...
    yellow "  ⚠ matplotlib not installed — math/chart/flowchart blocks degrade to text  (run: make.sh fix for guidance)"
  fi
...
# ── fix ────────────────────────────────────────────────────────────────────────
cmd_fix() {
  bold "Dependency guidance"
  echo ""
  echo "Install the following in the host environment:"
  echo "  Python packages : reportlab, pypdf, matplotlib"
  echo "  Node package    : playwright"
  echo "  Browser         : Chromium / Chrome / Edge, or set PLAYWRIGHT_CHROMIUM_EXECUTABLE"
  echo ""
  echo "Example host commands:"
  echo "  python3 -m pip install reportlab pypdf matplotlib"
  echo "  npm install playwright"
  echo "  npx playwright install chromium"
  echo ""
  yellow "This helper intentionally does not auto-install dependencies inside the plugin bundle."
  exit 0
}
